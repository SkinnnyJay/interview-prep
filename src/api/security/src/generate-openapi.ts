/**
 * Generate OpenAPI JSON for the Security API.
 * Writes openapi.json to the project root. Run with: npm run openapi
 * Uses JWT_SECRET=openapi-gen so the server module loads without exiting.
 */
...
async function main(): Promise<void> {
  ...
  await app.ready();
  const schema = (app as unknown as { swagger: () => unknown }).swagger();
